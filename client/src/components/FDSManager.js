...
class FDSManager extends Component {
  ...
  extractDuration = (duration) => {
    let returnString = "";
    let emptyDuration =
      duration == null || (!duration.hours && !duration.minutes);
    if (emptyDuration) {
      return "0 hour 0 minute";
    }
    if (duration.hours) {
      returnString += duration.hours;
      returnString += duration.hours > 1 ? " hours " : " hour ";
    }
    if (duration.minutes) {
      returnString += "" + duration.minutes;
      returnString += duration.minutes > 1 ? " minutes " : " minute ";
    }
    return returnString;
  };
  ...
  handleQuery = () => {
    if (
      this.state.month &&
      this.state.year &&
      this.validInput(this.state.month, this.state.year)
    ) {
  ...
  renderAreaDropdown = () => {
    ...
    for (let i = 0; i < areas.length; i++) {
      listOfOptions.push(
        <option key={areas[i]} values={areas[i]}>
          {areas[i]}
        </option>
      );
    }
  ...
  checkForNoMissingField = (category, type) => {
    let isStandardFieldsPresence =
      this.state.promoStartDay &&
      this.state.promoEndDay &&
      this.state.details &&
      this.state.promoCategory &&
    ...
    return (
      ...
      isMinimumValuePresent
    );
  };

  inputValidityCheck = () => {
    let isValidTriggerVal = this.checkValidTriggerValue(
      this.state.promoType,
    ...
    let isValidTriggerValueAndDiscountValue =
      isValidDiscountVal && isValidTriggerVal;
    let promoStartDay = new Date(
      this.state.promoStartYear,
      this.state.promoStartMonth - 1,
      this.state.promoStartDay
    );
    let promoEndDay = new Date(
      this.state.promoEndYear,
      this.state.promoEndMonth - 1,
      this.state.promoEndDay
    );
    let invaliPromotionEndDate =
      promoStartDay.getTime() > promoEndDay.getTime();
  ...
  getPromoStartAndEndTime = () => {
    return Promise.all([
      this.formatDateForQuery(
        this.state.promoStartDay,
        this.state.promoStartMonth,
        this.state.promoStartYear,
        this.state.promoStartTime
      ),
      this.formatDateForQuery(
        this.state.promoEndDay,
        this.state.promoEndMonth,
        this.state.promoEndYear,
        this.state.promoEndTime
      ),
    ]);
  };
    ...
    return (
      <Tabs className="centered">
        ...
        <Row>
          <Col>
            <Jumbotron>
              ...
              <p className="lead">
                You can view all the stats below, Have fun working!
              </p>
        ...
        >
          ...
          <Tab eventKey="promotion" titile="Promotion">
            Promotion
          </Tab>
        </TabList>
            ...
            <Table>
              ...
              <tbody>
                <tr key="report-date-content">
                  <td>
                    {this.state.displayMonth ? this.state.displayMonth : 0}
                  </td>
        ...
        <TabPanel class="tab-panel" className="container">
          <Row>
            <Col xs="auto">
              <InputGroup>
                <Input type="select" onChange={this.onDayDropdownSelected}>
                  {this.renderDayDropdown(
                    this.state.displayMonth,
                    this.state.displayYear
                  )}
                </Input>
                <Input type="select" onChange={this.onAreaDropdownSelected}>
                  {this.renderAreaDropdown()}
                </Input>
                <InputGroupAddon addonType="append">
                  <Button color="primary" onClick={this.handleLocationQuery}>
                    Enter
                  </Button>
                </InputGroupAddon>
              </InputGroup>
        ...
        <TabPanel class="tab-panel">
          <InputGroup className="container">
            <h2>Start date</h2>
            <InputGroupAddon addonType="append">
              <Input type="select" onChange={this.onPromoStartDayChanged}>
                {this.renderDayDropdown(
                  this.state.promoStartMonth,
                  this.state.promoStartYear
                )}
              </Input>
              <Input type="select" onChange={this.onPromoStartMonthChanged}>
                {this.renderMonthDropdown()}
              </Input>
              <Input type="select" onChange={this.onPromoStartYearChanged}>
                {this.renderYearDropdown()}
              </Input>
              <Input type="select" onChange={this.onPromoStartTimeChanged}>
          ...
          <InputGroup className="container">
            <h2>End date</h2>
            <InputGroupAddon addonType="append">
              <Input type="select" onChange={this.onPromoEndDayChanged}>
                {this.renderDayDropdown(
                  this.state.promoEndMonth,
                  this.state.promoEndYear
                )}
              </Input>
              <Input type="select" onChange={this.onPromoEndMonthChanged}>
                {this.renderMonthDropdown()}
              </Input>
              <Input type="select" onChange={this.onPromoEndYearChanged}>
                {this.renderYearDropdown()}
              </Input>
              <Input type="select" onChange={this.onPromoEndTimeChanged}>
                {this.renderTimeDropdown()}
              </Input>
            </InputGroupAddon>
          </InputGroup>
          <div className="container div">
            <Row>
              <textarea
                placeholder="Details of discount..."
                value={this.state.details}
                onChange={this.onDetailsChanged}
              />
              {this.state.deliveryPromo && (
                <Input
                  placeholder="Discount value"
                  onChange={this.onPromoDiscountValueChanged}
                ></Input>
              )}
              {this.state.deliveryPromo && (
                <Input
                  placeholder="Minimum value to apply discount"
                  onChange={this.onPromoMinValChanged}
                ></Input>
              )}
            </Row>
            <Row>
              <Col xs="auto">
                <InputGroup>
                  <Input type="select" onChange={this.onPromoCategoryChanged}>
                    {this.renderPromoCategory()}
                  </Input>
                  <Input type="select" onChange={this.onPromoTypeChanged}>
                    {this.renderPromoType()}
                  </Input>
                  {this.state.isDisplayLimitPromo && (
                    <Input type="select" onChange={this.onPromoLimitChange}>
                      {this.renderPromoLimit()}
                    </Input>
                  )}
                  <InputGroupAddon addonType="append">
                    <Button color="primary" onClick={this.handleAddPromoButton}>
                      Add Promotion
                    </Button>
                  </InputGroupAddon>
                </InputGroup>
